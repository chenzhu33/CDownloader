...
import com.carelife.cdownloader.core.listener.SimpleDownloadingListener;
import com.carelife.cdownloader.core.display.BitmapDisplayer;
import com.carelife.cdownloader.core.display.FakeBitmapDisplayer;
...
import com.carelife.cdownloader.core.listener.SimpleImageLoadingListener;
...
import com.carelife.cdownloader.utils.ImageSizeUtils;
import com.carelife.cdownloader.utils.L;

/**
 * Singletone for image loading and displaying at {@link ImageView ImageViews}<br />
 * <b>NOTE:</b> {@link #init(CDownloaderConfiguration)} method must be called before any other method.
 * 
...
public class CDownloader {
	...
	public static final String TAG = CDownloader.class.getSimpleName();
	...
	private CDownloaderConfiguration configuration;
	private CDownloaderEngine engine;

	private final ImageLoadingListener emptyListener = new SimpleImageLoadingListener();
	private final BitmapDisplayer fakeBitmapDisplayer = new FakeBitmapDisplayer();

	private volatile static CDownloader instance;

	/** Returns singleton class instance */
	public static CDownloader getInstance() {
		if (instance == null) {
			synchronized (CDownloader.class) {
				if (instance == null) {
					instance = new CDownloader();
				}
			}
		}
		return instance;
	 ...
	 * 
	 * @param configuration {@linkplain CDownloaderConfiguration ImageLoader configuration}
	 * @throws IllegalArgumentException if <b>configuration</b> parameter is null
	 */
	public synchronized void init(CDownloaderConfiguration configuration) {
		if (configuration == null) {
			throw new IllegalArgumentException(ERROR_INIT_CONFIG_WITH_NULL);
		}
		if (this.configuration == null) {
			if (configuration.loggingEnabled) L.d(LOG_INIT_CONFIG);
			engine = new CDownloaderEngine(configuration);
	...
	/**
	 * Returns <b>true</b> - if ImageLoader {@linkplain #init(CDownloaderConfiguration) is initialized with
	 ...
	 * Default {@linkplain DisplayImageOptions display image options} from {@linkplain CDownloaderConfiguration
	 * configuration} will be used.<br />
	 * <b>NOTE:</b> {@link #init(CDownloaderConfiguration)} method must be called before this method call
	 * 
	 * @param uri Image URI (i.e. "http://site.com/image.png", "file:///mnt/sdcard/image.png")
	 * @param imageView {@link ImageView} which should display image
	 * 
	 * @throws IllegalStateException if {@link #init(CDownloaderConfiguration)} method wasn't called before
	 ...
	 *            {@linkplain CDownloaderConfiguration.Builder#defaultDisplayImageOptions(DisplayImageOptions) from
	...
	public void downloadFile(String uri, DownloadingListener listener) {
		checkConfiguration();
		if(listener == null) {
			listener = new SimpleDownloadingListener();
		}
		...
		FileDownloadingInfo downloadingInfo = new FileDownloadingInfo(uri, listener, engine.getLockForUri(uri));
		DownloadTask downloadTask = new DownloadTask(engine, downloadingInfo, new Handler());
	 ...
	 * You can {@linkplain #init(CDownloaderConfiguration) init} ImageLoader with new configuration after calling this
	 * method.
	 */
	public void destroy() {
